class SegmentTree:
    ...
    def __build_tree(self, low, high, pos):
        ...
        self._tree[pos] = min(self._tree[self._left_node(pos)], self._tree[self._right_node(pos)])
    ...
    def range_quiry(self, ql, qr):
        return self._find_min(0, self.hight, ql, qr, self.root)


    def _find_min(self, low, high, ql, qr, pos):
        ...
        mid = (low + high) // 2
        return min(
            self._find_min(low, mid, ql, min(qr, mid), self._left_node(pos)),
            self._find_min(mid + 1, high, max(ql, mid+1), qr, self._right_node(pos)),
            )

input = [5,3,-1,5,10,2,7,9,20]
st = SegmentTree(input)
assert st.range_quiry(1, 6) == -1
assert st.range_quiry(1, 1) == 3 
assert st.range_quiry(0, 2) == -1
assert st.range_quiry(0, 8) == -1
